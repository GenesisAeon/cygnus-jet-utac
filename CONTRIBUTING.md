...
- Format and lint with `ruff check cygnus_jet_utac tests`.
- Type-check with `mypy cygnus_jet_utac`.
...
`to_zenodo_record`) on `CygnusJetUTAC`. Any change to these methods'
...
If your contribution touches the UTAC/CREP model, calibration targets, or
falsifiable predictions, please cite the source (e.g. Prabu et al. 2026)
and clearly mark speculative vs. validated claims.
